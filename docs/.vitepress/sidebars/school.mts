export default [
    {text:'2024年社会实践', items:[
        {text:"鬼子进村", link:'/school/guizijincun/index.md'},
        {text:"超雄neiger", link:'/school/chaoxiongnige/index.md'},
        {text:"老六体委", link:'/school/laoliutiwei/index.md'},
        ],},
    {text:'2025年篮球班赛', items:[
        {text:"5班 vs 18班", link:'/school/vs18/index.md'},
        {text:"5班 vs 14班", link:'/school/vs14/index.md'},
        {text:"5班 vs 20班", link:'/school/vs20/index.md'},
        {text:"5班 vs 8班", link:'/school/vs8/index.md'},
        ],},
    {text:'2025年五四青年朗诵比赛', link:'/school/langsongbisai/index.md'},
]